...
4. 按技术栈拆分创建带 `task_doc` 的修复任务，任务描述写明报告路径和该技术栈下的全部问题 ID；要求开发专家修复报告中属于其技术栈的所有问题。
5. 消失的问题视为已修复；新问题从 0 次计数；位置/根因一致的问题沿用原计数。
6. 每轮修复后重新完整测试/复审；循环持续直到所有问题均已修复或单个问题修复 3 次后放弃。
...
- CLI/脚本类项目：跳过 CSRF/XSS/前端性能相关指令
- API 服务（无模板）：跳过模板安全和前端 CWV 指令
...
| 测试报告缺少拒绝/边界/滥用路径 | 补调 liam 或 mia 针对缺失维度补测 | 用成功路径测试结果推进 |
| 审查报告缺少业务不变量或文档一致性 | 补调 alex 覆盖权限生命周期、生产准入、测试有效性和 docs 对照 | 只看普通代码质量结论 |
