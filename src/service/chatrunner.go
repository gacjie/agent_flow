...
func (r *ChatRunner) Run(ctx context.Context, cfg RunConfig, eventCh chan<- StreamEvent) {
	...
	sessionSvc.SaveMessage(cfg.ConversationID, "user", cfg.UserMessage, nil, "", "", 0, "")
...
// resolveModelClients 根据 modelID 构建模型+客户端列表
func (r *ChatRunner) resolveModelClients(modelID string) ([]modelClient, error) {
	if modelID == "auto" {
		...
		for i := range models {
			...
			if err != nil {
				slog.Warn("创建 auto 模型客户端失败，跳过", "model_id", models[i].ModelID, "error", err)
				continue
			}
			result = append(result, modelClient{Model: &models[i], Client: client})
		}
		if len(result) == 0 {
			return nil, fmt.Errorf("没有可用的自动切换模型")
		}
		return result, nil
	}

	m, err := r.ModelService.ResolveModel(modelID)
	if err != nil {
		return nil, err
	}
	client, err := r.ModelService.CreateClientFromModel(m)
	if err != nil {
		return nil, fmt.Errorf("创建 LLM 客户端失败: %w", err)
	}
	return []modelClient{{Model: m, Client: client}}, nil
}
